...
INSERT INTO products2 (product_name, price, category)
VALUES
...
select p.product_name, w.warehouse_name, i.quantity from inventory i
join products2 p on i.product_id = p.product_id
join warehouses w ON i.warehouse_id = w.warehouse_id
where p.category = 'Electronics'
order by i.quantity desc
limit 3 offset 1;

select p.product_name, s.supplier_name, w.warehouse_name, i.quantity from inventory i
join products2 p on i.product_id = p.product_id
join warehouses w on i.warehouse_id = w.warehouse_id
...
select p.product_name, p.price, w.warehouse_name, w.location, i.quantity, s.supplier_name from products2 p
...
select p.product_name, p.price, w.warehouse_name, i.quantity, s.supplier_name, p.category from products2 p
join inventory i on p.product_id = i.product_id
join warehouses w on i.warehouse_id = w.warehouse_id
join suppliers s on p.product_id = s.product_id
