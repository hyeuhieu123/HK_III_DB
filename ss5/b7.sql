...
select name,email,course_name,enrollment_date from
...
where fee>1000000
or courses.course_id not in (select course_id from enrollments)
order by fee desc,course_name;


select name,email,course_name,enrollment_date from
students left join enrollments on students.student_id=enrollments.student_id
right join courses on courses.course_id=enrollments.course_id
where fee>1000000
or email is null
